...
def export_pdf_report():
    ...
    # Title and Firm Information
    pdf.cell(200, 10, txt="Maham for Professional Services", ln=True, align="C")
    pdf.cell(200, 10, txt="Audit Report Summary", ln=True, align="C")
    pdf.ln(10)

    # Report Date and Period Covered
    pdf.cell(200, 10, txt=f"Report Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=True, align="L")
    pdf.cell(200, 10, txt=f"Period Covered: {st.session_state.year_audited}", ln=True, align="L")
    pdf.ln(10)

    # Objective of the Report
    pdf.cell(200, 10, txt="Objective of the Report:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt="The objective of this report is to provide an overview of the audit findings, including completeness, accuracy, and high-risk criteria testing. The report highlights discrepancies, unusual patterns, and actionable recommendations to mitigate risks.")
    pdf.ln(10)

    # General Ledger Overview
    pdf.cell(200, 10, txt="General Ledger Overview:", ln=True, align="L")
    ...
    pdf.cell(200, 10, txt=f"Total Transactions: {len(st.session_state.processed_df)}", ln=True, align="L")
    pdf.ln(10)

    # Testing Results
    pdf.cell(200, 10, txt="Testing Results:", ln=True, align="L")
    pdf.cell(200, 10, txt="Tests Performed:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt="1. Completeness Check\n2. Accuracy Check\n3. High-Risk Criteria Testing (e.g., public holidays, rounded numbers, unusual users, post-closing entries)")
    pdf.cell(200, 10, txt="Findings:", ln=True, align="L")
    if st.session_state.completeness_check_passed:
        pdf.multi_cell(0, 10, txt="Completeness check passed. Maximum discrepancy is within the allowed tolerance of 5.")
    else:
        max_discrepancy = st.session_state.completeness_check_results["Discrepancy"].abs().max()
        pdf.multi_cell(0, 10, txt=f"Completeness check failed. Maximum discrepancy ({max_discrepancy}) exceeds the allowed tolerance of 5.")
    pdf.cell(200, 10, txt="Risk Indicators:", ln=True, align="L")
    if st.session_state.high_risk_entries is not None and not st.session_state.high_risk_entries.empty:
        pdf.multi_cell(0, 10, txt=f"Found {len(st.session_state.high_risk_entries)} high-risk entries meeting the defined criteria.")
    else:
        pdf.multi_cell(0, 10, txt="No high-risk entries were identified.")
    pdf.ln(10)

    # Materiality and Thresholds
    pdf.cell(200, 10, txt="Materiality and Thresholds:", ln=True, align="L")
    pdf.cell(200, 10, txt=f"Materiality Threshold: {st.session_state.auth_threshold}", ln=True, align="L")
    if st.session_state.high_risk_entries is not None and not st.session_state.high_risk_entries.empty:
        threshold_breaches = st.session_state.high_risk_entries[
            (st.session_state.high_risk_entries["Debit Amount (Dr)"] >= st.session_state.auth_threshold * 0.9) |
            (st.session_state.high_risk_entries["Credit Amount (Cr)"] >= st.session_state.auth_threshold * 0.9)
        ]
        pdf.cell(200, 10, txt=f"Threshold Breaches: {len(threshold_breaches)} entries breached the materiality threshold.", ln=True, align="L")
    else:
        pdf.cell(200, 10, txt="No threshold breaches were identified.", ln=True, align="L")
    pdf.ln(10)

    # High-Risk Criteria & Fraud Detection
    pdf.cell(200, 10, txt="High-Risk Criteria & Fraud Detection:", ln=True, align="L")
    pdf.cell(200, 10, txt="Criteria and Reasoning:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt="The following high-risk criteria were applied:\n1. Public Holidays\n2. Rounded Numbers\n3. Unusual Users\n4. Post-Closing Entries\n5. Entries Just Below Authorization Threshold\n6. 99999 Pattern\n7. Suspicious Keywords\n8. Seldomly Used Accounts")
    pdf.cell(200, 10, txt="Fraud Detection Insights:", ln=True, align="L")
    if st.session_state.high_risk_entries is not None and not st.session_state.high_risk_entries.empty:
        pdf.multi_cell(0, 10, txt=f"Identified {len(st.session_state.high_risk_entries)} entries flagged for further investigation based on the applied criteria.")
    else:
        pdf.multi_cell(0, 10, txt="No entries were flagged for fraud detection.")
    pdf.ln(10)

    # Conclusion and Recommendations
    pdf.cell(200, 10, txt="Conclusion and Recommendations:", ln=True, align="L")
    pdf.cell(200, 10, txt="Summary of Findings:", ln=True, align="L")
    if st.session_state.completeness_check_passed:
        pdf.multi_cell(0, 10, txt="The completeness check passed with no significant discrepancies.")
    else:
        pdf.multi_cell(0, 10, txt="The completeness check failed. Further investigation is required to resolve discrepancies.")
    if st.session_state.high_risk_entries is not None and not st.session_state.high_risk_entries.empty:
        pdf.multi_cell(0, 10, txt=f"Identified {len(st.session_state.high_risk_entries)} high-risk entries. Further investigation is recommended.")
    else:
        pdf.multi_cell(0, 10, txt="No high-risk entries were identified.")
    pdf.cell(200, 10, txt="Actionable Recommendations:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt="1. Investigate flagged high-risk entries.\n2. Review and resolve discrepancies in the completeness check.\n3. Strengthen internal controls to mitigate risks.")
    pdf.ln(10)

    # Supporting Visuals and Data
    pdf.cell(200, 10, txt="Supporting Visuals and Data:", ln=True, align="L")
    pdf.cell(200, 10, txt="Charts and Graphs:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt="Visualizations of transaction patterns are available in the application interface.")
    pdf.cell(200, 10, txt="Data Integrity Check:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt="Data cleansing and validation were performed during the audit process.")
    pdf.ln(10)

    # Audit Sign-off
    pdf.cell(200, 10, txt="Audit Sign-off:", ln=True, align="L")
    pdf.cell(200, 10, txt="Audit Conclusion:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt="This report provides a summary of the audit findings and recommendations. Further investigation may be required based on the identified risks.")
    pdf.cell(200, 10, txt="Auditor Information:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt=f"Generated By: {st.session_state.logged_in_user}\nRole: Auditor\nDisclaimer: This report is for internal use only.")
    pdf.ln(10)

    # Custom Notes for MAHAM DATA DEEP ANALYZER Integration
    pdf.cell(200, 10, txt="Custom Notes for MAHAM DATA DEEP ANALYZER Integration:", ln=True, align="L")
    pdf.cell(200, 10, txt="Structured Data Points:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt="Risk Levels: High, Medium, Low\nFraud Indicators: Rounded Numbers, Unusual Users, Post-Closing Entries\nError Types: Discrepancies, Threshold Breaches")
    pdf.cell(200, 10, txt="Specific Fields or Tags for MAHAM DATA DEEP ANALYZER Analysis:", ln=True, align="L")
    pdf.multi_cell(0, 10, txt="Account Number, Transaction ID, Date, Debit Amount (Dr), Credit Amount (Cr)")
