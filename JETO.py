...
if 'audited_client_name' not in st.session_state:  # New session state variable
    st.session_state.audited_client_name = ""
if 'year_audited' not in st.session_state:  # New session state variable
...
def export_pdf_report():
    ...
    pdf.cell(200, 10, txt="Completeness Check Results:", ln=True, align="L")
    pdf.set_font("Arial", size=10)
    for index, row in st.session_state.completeness_check_results.iterrows():
        pdf.cell(200, 10, txt=f"Account {row['Account Number']}: Expected {row['Expected_Ending_Balance']}, Actual {row['Ending Balance']}, Discrepancy {row['Discrepancy']}", ln=True, align="L")
...
def main_app():
    ...
    # Export PDF Report
    if st.session_state.completeness_check_results is not None:
